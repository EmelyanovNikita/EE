 ...
 int main (int argc, char * argv[])
{
	...
	//создадим новый семафор с начальным значением NUM_PROC ( количество процессов, в нашей ситуации, могущих одновременно читать)
	if ( (sem = sem_open(SEMAPHORE_NAME, O_CREAT, 0777, NUM_PROC)) == SEM_FAILED ) {
        perror("sem_open");
        exit(EXIT_FAILURE);
    }
		...
		switch ( pid )
		{
			...
			default: // родитель
			{
				...
				if ( i < num )
				{
					...
					for (int j = 0; j < NUM_PROC; ++j) // так как никто не должен читать
					{
						//заблокируем семафор
						if( sem_wait(sem) == -1 )
						{
