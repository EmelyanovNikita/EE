...
void* receive_from_server(void* arg) 
{
    ...
    while (1) 
	{
		...
		size_t data_size;
		recv(client_socket, &data_size, sizeof(data_size), 0); // Читаем размер данных

		char* buffer = malloc(data_size + 1); // Выделяем память под данные
		ssize_t result = recv(client_socket, buffer, data_size, 0); // Читаем данные
		buffer[data_size] = '\0'; // Добавляем нулевой символ
		
		
		printf("Было получено %zu байт\n", data_size);
